...
protected:
    ...
    // 이벤트 기반 캐시 관리 함수들
    void RegisterZombie(ANS_ZombieBase* Zombie);
    void UnregisterZombie(ANS_ZombieBase* Zombie);
    void RegisterPlayer(ANS_PlayerCharacterBase* Player);
    void UnregisterPlayer(ANS_PlayerCharacterBase* Player);
    void RegisterSpawner(AANS_ZombieSpawner* Spawner);
    void UnregisterSpawner(AANS_ZombieSpawner* Spawner);

    // 캐시 정리 함수들 (무효한 참조만 제거)
    void CleanupZombieCache();
    void CleanupPlayerCache();
    void CleanupSpawnerCache();

    // 초기 캐시 구축 함수 (BeginPlay에서 한 번만 호출)
    void InitializeActorCaches();
